...
function slugifyHeading(text: string): string {
  ...
  // for headings, e.g. when there are <code> spans inside).
 ...
 * conv 42fd2cb8 had 46 such leaks). Runs of 1 or 3+ `*` are left untouched,
 * and `**` runs inside code spans / math are already placeholder-protected
 * by extractMath before this runs. The inserted space is invisible in CJK
 * layout and makes the closing run right-flanking so marked closes the bold.
 */
