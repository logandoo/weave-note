...
async function handleDeleteNotebook(id: string) {
  ...
  if (!await showConfirm({ message: '确定要删除这个笔记本吗？所有笔记也会被删除。', danger: true, confirmText: '删除' })) {
    return
  }
...
@media (min-width: 768px) {
  ...
  .stack-column > .notebook-row {
    ...
    box-shadow: 0 -2px 8px rgba(90, 130, 60, 0.08), 0 4px 14px rgba(90, 130, 60, 0.12);
    border-radius: 18px;
    contain: paint;
  }
  ...
  .stack-column > .notebook-row:hover {
    ...
    box-shadow: 0 -4px 16px rgba(90, 130, 60, 0.12), 0 18px 38px rgba(0, 0, 0, 0.22);
  }
