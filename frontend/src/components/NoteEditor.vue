<template>
  <div class="note-editor-page">
    ...
    <div class="editor-toolbar">
      ...
      <button class="toolbar-btn" @mousedown.prevent="wysiwygEditorRef?.saveEditorSelection()" @click="handleRestartNumbering()" title="重新编号">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          <path d="M1 4v6h6"/>
      ...
      <!-- weave-note 无 ASR 后端：语音输入入口禁用（代码主体保留） -->
      <button v-if="false" class="toolbar-btn" @click="startVoiceNote" :disabled="isProcessing" title="语音输入">
...
.preview-content :deep(.hljs-tag) { color: #d73a49; }
...
.preview-content :deep(.hljs-type) { color: #032f62; }
.preview-content :deep(.hljs-comment),
.preview-content :deep(.hljs-quote) { color: #6a737d; font-style: italic; }
.preview-content :deep(.hljs-number) { color: #005cc5; }
.preview-content :deep(.hljs-function) { color: #6f42c1; }
.preview-content :deep(.hljs-variable),
.preview-content :deep(.hljs-params) { color: #e36209; }
