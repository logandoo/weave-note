...
case "$db_type" in
  sqlite)
    echo "weave-note 数据库类型: sqlite"
    echo "  库文件 backend/*.db 首次启动自动创建，无需 PostgreSQL"
    precreate_tables
    ;;
  postgres)
    echo "weave-note 数据库类型: postgres"
    ...
    if psql -U "$PGUSER" -h "$PGHOST" -p "$PGPORT" -d postgres -tAc \
        "SELECT 1 FROM pg_database WHERE datname='weave_note'" | grep -q 1; then
      echo "  PG 数据库 weave_note 已存在，跳过创建"
    else
      createdb -U "$PGUSER" -h "$PGHOST" -p "$PGPORT" weave_note
      echo "  PG 数据库 weave_note 创建完成"
    fi
    precreate_tables
    ;;
  *)
